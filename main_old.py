...
def gpr_processing(path, filename):
    '''
    GPR 읽기 및 전처리 파이프라인
...
def main():
    ...
    parser.add_argument('--start_m', type=float, required=True, help='이미지 자를 시작 인덱스')
    parser.add_argument('--length_m', type=float, required=True, help='자를 길이')
    ...
    # 파일 읽기 및 전처리
    rd3 = gpr_processing(args.path, args.filename)

    # 이미지 자르기
    rd3 = cutRd3(rd3, args.start_m, args.length_m, args.path, args.filename)
